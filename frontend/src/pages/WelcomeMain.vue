...
const login = () => {
    ...
    axios
        ...
        .then(resp => {
            ...
            userData.user.expiry = resp.data.expiry
            userData.user.token = resp.data.token
        ...
        <div class="row">
          <div class="col-md-8 offset-md-2">
            <form class="df" @submit.prevent="login" v-show="!waitingForServerResponse">
              ...
              <button v-if=!userData.user.token type="submit" class="btn btn-success">Belépés</button>
            </form>
          </div>
        </div>
      </div>
